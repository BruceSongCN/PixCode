...
不要修改业务代码，不要伪造 `review.md` 或 `verification.md` 的通过结论。
```
...
`design.md` 的“模型变化”和“实体字段定义”章节应定义独立的新模型，而不是只增加一个字段。
...
每个持久化实体必须独立逐字段定义。不能把 `DeviceId/AssigneeId`、时间字段组或其他字段集合合并成一行。

#### `InspectionTask`（巡检任务）

| 字段 | 类型 | 空值 | 默认长度 | 中文含义/XML 字段说明 | 来源/规则 |
| --- | --- | --- | --- | --- | --- |
| `Id` | `Guid` | 否 | - | 巡检任务主键 | 系统生成 |
| `TaskNo` | `string` | 否 | `STR_CODE_LENGTH`（64） | 巡检任务编号 | 服务端生成，全局唯一 |
| `DeviceId` | `Guid` | 否 | - | 设备 Id | 用户选择的有效设备 |
| `AssigneeId` | `Guid` | 否 | - | 执行人用户 Id | 用户选择且具备执行权限 |
| `Deadline` | `DateTime` | 否 | - | 截止时间 | 用户输入，不得早于发布时间 |
| `Status` | `InspectionTaskStatusEnum` | 否 | - | 巡检任务状态 | 草稿、待执行、已完成或已取消 |
| `Version` | `int` | 否 | - | 并发版本 | 每次有效修改后递增 |
| `PublishedAt` | `DateTime?` | 是 | - | 发布时间 | 首次发布时由服务端写入 |
| `CompletedAt` | `DateTime?` | 是 | - | 完成时间 | 全部项目完成后由服务端写入 |
| `CancelledAt` | `DateTime?` | 是 | - | 取消时间 | 任务取消后由服务端写入 |

#### `InspectionItem`（巡检项目）

| 字段 | 类型 | 空值 | 默认长度 | 中文含义/XML 字段说明 | 来源/规则 |
| --- | --- | --- | --- | --- | --- |
| `Id` | `Guid` | 否 | - | 巡检项目主键 | 系统生成 |
| `TaskId` | `Guid` | 否 | - | 巡检任务 Id | 关联 `InspectionTask.Id` |
| `Name` | `string` | 否 | `STR_NAME_LENGTH`（64） | 巡检项目名称 | 创建任务时输入 |
| `SortOrder` | `int` | 否 | - | 显示顺序 | 同一任务内稳定排序 |
| `Result` | `InspectionResultEnum?` | 是 | - | 巡检结果 | 执行阶段填写；完成前必须有值 |
| `Remark` | `string?` | 是 | `STR_DESC_LENGTH`（256） | 异常说明 | 结果异常时条件必填 |

字段中文说明必须同步用于实体、DTO、公开契约 XML 注释和 `Description`。字符串长度引用目标仓库真实常量；无法确认时写“待确认”，不能猜测。
...
`design.md` 的“流程设计”章节应至少包含创建发布和执行完成两条主流程。
...
`design.md` 的“API与共享契约”章节应记录稳定语义，并遵守现有项目 API 风格。以下只是语义示例：
...
`test-plan.md` 应从 Requirement 和 Scenario 推导，而不是从历史测试目录猜测。
...
- `proposal.md`；
- `specs/device-inspection-task/spec.md`；
- `design.md`，且功能、流程、模型、API、交互和质量维度均有结论；
- `test-plan.md`；
- 初始状态为“待评审”的 `review.md`。

尚未正式评审时不要生成可执行的 `tasks.md`，也不要把 `review.md` 标记为通过。
...
```text
使用 $pixcode-workflow review device-inspection-task。

请逐项评审需求范围、功能、流程、模型、API、交互、测试方案和多 Target 发布边界，
把问题、决定、责任角色和结论记录到 review.md。
不要自行假设业务负责人已经批准。
```

评审真实通过后再生成 `tasks.md`。任务应按共享契约、backend、frontend-web、跨 Target 联调和交付收口分组，并关联 Requirement / Scenario 与完成条件。
...
确认 `review.md` 已通过或有条件通过、没有未关闭阻断问题，并搜索旧结论确保资产间没有冲突。只有此时才进入实现。
...
`verification.md` 的验证和最终交付结论只能基于真实执行与验收：
...
在设计评审未通过、任务未全部完成或 `verification.md` 不存在时执行：
...
不要通过伪造任务状态或验证结果使归档通过。PixCode 不提供绕过任务、评审或真实验证门禁的归档参数。
...
Agent 必须根据 `pixcode.yaml` 把同步后的完整需求、归档设计和既有功能资产合并为当前完整结论，然后执行：
...
Get-Content openspec/specs/device-inspection-task/spec.md
Get-ChildItem pix-specs/设备管理/巡检管理/设备巡检任务
```
...
- change 已进入归档；
- 当前 spec 保留最终有效需求；
- `pix-specs/设备管理/巡检管理/设备巡检任务/` 包含当前完整结论和 OpenSpec 映射；
- 验证报告和证据索引可追溯；
- 所有严格校验通过。
...
| 内容 | 来源 |
| --- | --- |
| OpenSpec 当前需求事实 | `openspec/specs/device-inspection-task/spec.md` |
| 当前完整需求、设计、模型、流程、契约和测试策略 | `pix-specs/设备管理/巡检管理/设备巡检任务/` |
| 每轮过程、验证、验收和例外 | 归档 Change |
| 后端代码与测试 | backend commit / PR |
| 前端代码与测试 | frontend-web commit / PR |
| 实际验证、验收和交付决定 | `verification.md` |
| 证据 | `verification.md` 中的 evidence 索引 |
| 风险和未执行项 | `verification.md` 的失败与遗留风险 |
...
- [ ] 跨 Target 主流程真实执行通过。
- [ ] 失败和未执行项没有隐藏。
- [ ] `review.md` 分维度记录真实评审结论且没有未关闭阻断问题。
- [ ] `verification.md` 能追溯到 Requirement 和证据，并记录最终交付决定。
- [ ] 未提交凭证、生产数据和大型运行产物。
- [ ] 当前 spec 表达最终有效行为。
- [ ] change 已归档。
- [ ] 三个仓库的提交和发布顺序清楚。
